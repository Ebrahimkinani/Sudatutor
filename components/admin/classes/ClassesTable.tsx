"use client";

import { AdminTable, Column } from "@/components/admin/AdminTable";
import { ClassActions } from "@/components/admin/ClassActions";

interface ClassItem {
    id: string;
    name: string;
    createdAt: Date;
    isActive: boolean;
    stats: {
        subjectsCount: number;
        chatsCount: number;
        messagesCount: number;
        activeUsersCount: number;
    };
}

interface ClassesTableProps {
    data: ClassItem[];
}

export function ClassesTable({ data }: ClassesTableProps) {
    const columns: Column<ClassItem>[] = [
        { key: "name", label: "Class Name", sortable: true, className: "font-medium" },
        { key: "subjectsCount", label: "Subjects", render: (getRow) => getRow.stats.subjectsCount },
        { key: "chatsCount", label: "Chats", render: (getRow) => getRow.stats.chatsCount },
        { key: "messagesCount", label: "Messages", render: (getRow) => getRow.stats.messagesCount },
        { key: "activeUsersCount", label: "Active Users", render: (getRow) => getRow.stats.activeUsersCount },
        { key: "createdAt", label: "Created At", sortable: true },
        {
            key: "isActive",
            label: "Status",
            render: (item) => (
                <span className={`px-2 py-1 rounded-full text-xs ${item.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                    {item.isActive ? 'Active' : 'Inactive'}
                </span>
            )
        },
        {
            key: "actions",
            label: "Actions",
            className: "w-[100px]",
            render: (item) => <ClassActions item={item} />
        }
    ];

    return (
        <AdminTable
            data={data}
            columns={columns}
        />
    );
}
